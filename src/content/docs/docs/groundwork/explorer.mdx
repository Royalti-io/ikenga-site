...
![The groundwork explorer with the plan board open in a tab, Kanban view — file tree on the left, two tabs open, the live board.html rendered in the viewer, file metadata in the right rail](/assets/groundwork/explorer.png)
...
![The design-system profile's explorer in gallery mode with a design selected — a locked homepage direction previewing live in the viewer](/assets/groundwork/explorer-gallery.png)
...
![The cross-plan plans index — three plans, one per profile, each card with a work-package progress bar, profile chip, drift indicator, and drill-in buttons to that plan's explorer and board](/assets/groundwork/plans-index.png)
